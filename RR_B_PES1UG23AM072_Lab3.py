...
def get_entropy_of_dataset(tensor: torch.Tensor):
    ...
    probabilities = counts.float() / target.size(0)
...
def get_avg_info_of_attribute(tensor: torch.Tensor, attribute: int):
    ...
    unique_values, counts = torch.unique(attribute_values, return_counts=True)
    avg_info = 0.0  
    for val in unique_values:
        subset = tensor[attribute_values == val]
        subset_entropy = get_entropy_of_dataset(subset)
        weight = subset.size(0) / total_count
...
def get_selected_attribute(tensor: torch.Tensor):
    ...
    num_attributes = tensor.size(1) - 1  # Exclude target column
    info_gains = {} 
    for attribute in range(num_attributes):
        info_gains[attribute] = get_information_gain(tensor, attribute)
